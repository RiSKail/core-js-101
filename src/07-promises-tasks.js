...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((res, rej) => {
    if (isPositiveAnswer && typeof isPositiveAnswer === 'boolean') res('Hooray!!! She said "Yes"!');
    if (!isPositiveAnswer && typeof isPositiveAnswer === 'boolean') res('Oh no, she said "No".');
...
function chainPromises(array, action) {
  return new Promise((res) => {
    const arr = [];
    array.map((e) => e.then((elem) => arr.push(elem)).catch((er) => er));
    res(arr);
